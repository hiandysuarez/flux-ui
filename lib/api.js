 ...
 * @param {string} strategy - "orb" or "llm"
 */
export function fetchSettingsSuggestions(days = 30, strategy = 'orb') {
  return authJson(`${mustBase()}/api/user/settings/suggestions?days=${days}&strategy=${strategy}`);
}
 ...
 * @param {string} strategy - "orb" or "llm"
 */
export function runBacktest(settings, days = 30, compareToCurrent = true, strategy = 'orb') {
 ...
 * @param {string} strategy - "orb" or "llm"
 */
export function fetchQuickBacktest(days = 30, strategy = 'orb') {
  return authJson(`${mustBase()}/api/user/backtest/quick?days=${days}&strategy=${strategy}`);
}
...
export function fetchStrategies() {
  return getJson(`${mustBase()}/api/strategies`);
}

/**
 * Get complete strategy configuration (global, LLM, strategy-specific settings)
 */
export function fetchStrategyConfig() {
  return authJson(`${mustBase()}/api/strategies/config`);
}

/**
 * Set active strategy for current user (requires auth)
 * @param {string|null} strategy - 'orb', 'momentum', or null for LLM-only
 */
export function setActiveStrategy(strategy) {
  return authJson(`${mustBase()}/api/user/strategy`, {
    method: 'POST',
    body: JSON.stringify({ active_strategy: strategy }),
  });
}

/**
 * Get settings schema for a specific strategy
 * @param {string} strategy - 'orb', 'momentum', 'llm', or 'global'
 */
export function fetchStrategySettings(strategy) {
  return getJson(`${mustBase()}/api/strategies/settings/${strategy}`);
}

/* =========================
   ORB STRATEGY ENDPOINTS
   ========================= */

/**
 * Get ORB tracker status for all symbols
 */
export function fetchORBStatus() {
  return getJson(`${mustBase()}/api/orb/status`);
}

/**
 * Get ORB settings (admin-level config)
 */
export function fetchORBSettings() {
  return authJson(`${mustBase()}/api/orb/settings`);
}

/**
 * Save ORB settings (requires auth)
 */
export function saveORBSettings(settings) {
  return authJson(`${mustBase()}/api/orb/settings`, {
    method: 'POST',
...
/**
 * Get ORB analytics/performance breakdown
 */
export function fetchORBAnalytics(startDate) {
  const params = new URLSearchParams();
  if (startDate) params.set('start_date', startDate);
  const qs = params.toString() ? `?${params.toString()}` : '';
  return getJson(`${mustBase()}/api/orb/analytics${qs}`);
}

/**
 * Get ORB trades (filtered from main trades)
 */
export function fetchORBTrades({ limit = 50, startDate, endDate, symbol } = {}) {
  const params = new URLSearchParams();
  params.set('limit', limit);
  if (startDate) params.set('start_date', startDate);
  if (endDate) params.set('end_date', endDate);
  if (symbol) params.set('symbol', symbol);
  return getJson(`${mustBase()}/api/orb/trades?${params.toString()}`);
}
