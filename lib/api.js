 ...
 * Retries up to maxRetries times for transient errors.
 */
async function fetchWithRetry(url, options = {}, maxRetries = 2) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!r.ok) {
        const error = new Error(`${r.status} ${r.statusText} :: ${text.slice(0, 200)}`);
        error.status = r.status;
        error.responseText = text;

        // Retry on transient errors
        if (attempt < maxRetries && isRetryableError(error, r.status)) {
          const delay = Math.min(1000 * Math.pow(2, attempt), 5000);
          console.warn(`[API] Retry ${attempt + 1}/${maxRetries} for ${url} (${r.status}), waiting ${delay}ms`);
      ...
      if (attempt < maxRetries && isRetryableError(e, e.status)) {
        const delay = Math.min(1000 * Math.pow(2, attempt), 5000);
        console.warn(`[API] Retry ${attempt + 1}/${maxRetries} for ${url}: ${e.message}`);
        await new Promise(resolve => setTimeout(resolve, delay));
        lastError = e;
        continue;
      }
 ...
 * If the token is expired, attempts to refresh the session once.
 */
      ...
      try {
        ...
        if (supabase) {
          ...
          if (!error && data?.session) {
            ...
            return await fetchWithRetry(url, { ...options, headers }, 0);
          }
        }
      } catch (refreshError) {
...
export function fetchTradeHistory({ page = 1, limit = 50, symbol, side, win, from, to, strategy } = {}) {
