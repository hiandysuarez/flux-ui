...
export function fetchSettingsSuggestions(days = 30) {
  return authJson(`${mustBase()}/api/user/settings/suggestions?days=${days}`);
}

/**
 * Run a backtest comparing settings (requires auth)
 */
export function runBacktest(settings, days = 30, compareToСurrent = true) {
  return authJson(`${mustBase()}/api/user/backtest`, {
    method: 'POST',
    body: JSON.stringify({
      settings,
      days,
      compare_to_current: compareToСurrent,
    }),
  });
}
...
export function fetchQuickBacktest(days = 30) {
  return authJson(`${mustBase()}/api/user/backtest/quick?days=${days}`);
}
